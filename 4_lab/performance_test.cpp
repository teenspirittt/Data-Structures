#include "src/CLHashTable.h"
#include "src/OAHashTable.h"
#include <cmath>
#include <stdlib.h>
#include <time.h> 
#include <vector>
...
int main() {
    ...
    cout << "step = " << step << "\n";
    for(int j = 0; j < 10; j++) {
        CLHashTable<int> cctable(m);
        for(int k = 0; cctable.GetSize() <= 1.5 * (double)m; k++) {
            n = cctable.GetSize();
            if (alphas.size() <= 1.5*m/step)
                alphas.push_back((double)n/(double)m);
            string key = newStr(10);
            cctable.Insert(key, 1);
            insert_c[k] += cctable.CountNodes();
            cctable.Get(key);
            search_c[k] += cctable.CountNodes();
            cctable.Remove(key);
            remove_c[k] += cctable.CountNodes();
            if (teor.size() <= 1.5*m/step)
                teor.push_back(teorCL(n, m));
            for (int i = 0; i < step; i++) {
                string str = newStr(10);
                keys.push_back(str);
                cctable.Insert(str, 0);
            }
        }
    }
    cout << "cctable, m = " << m << "\n";
